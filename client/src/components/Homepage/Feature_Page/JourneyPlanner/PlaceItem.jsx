...
const PlaceItem = ({
  ...
  const handleTimeChange = (index, key, newTime) => {
    if (key === "startTime") setTime(newTime);
    if (key === "endTime") setEndTime(newTime);
    handleChange(index, key, newTime);
  };
  ...
  return (
    <div className="w-full md:w-full mx-auto p-4">
      <ul className="space-y-4">
        {selectedPlaces.map((place, index) => (
          <li key={place.sr_no} className="p-4 bg-white border rounded-lg shadow-md">
            {place.isSummarized ? (
              <div className="summarized-view">
              ...
              <div className="space-y-3">
                <p className="font-semibold text-xl">{place.name}</p>
                ...
                <div className="flex flex-col gap-4 flex-wrap md:flex">
                  <TimePicker
                    label="Start Time"
                    time={place.startTime}
                ...
                <div className="space-y-3">
                  <label className="block font-medium">Select Activities</label>
                  <Select
                    isMulti
                    options={activities}
                    value={place.activities || []}
                    onChange={(activities) => handleChange(index, "activities", activities)}
                    placeholder="Select Activities"
                    className="w-full"
                  />
                  ...
                  {Object.entries(dropdowns).map(([key, value], idx) => (
                    <Dropdown
                      key={idx}
                      items={value}
                      labels={`Select ${key.charAt(0).toUpperCase() + key.slice(1)}`}
                      handleChange={handleChange}
                      indexval={index}
                      keyName={key}
                    />
                ...
                <label className="block">
                  ...
                  <input
                    type="number"
                    value={place.travelTime || ""}
                    onChange={(e) => handleChange(index, "travelTime", e.target.value)}
                    className="border p-2 rounded-md w-full"
                  />
                </label>
              </div>
            ...
            <div className="flex justify-between mt-3">
              <button className="text-red-500 font-semibold" onClick={() => handlePlaceSelection(place)}>
                Deselect
              </button>
              <button className="text-blue-500 font-semibold" onClick={() => toggleSummarize(index)}>
                {place.isSummarized ? "Edit" : "Save"}
              </button>
            </div>
          </li>
